...
KnexMigrator.prototype.init = function init(options) {
    ...
    return database.createDatabaseIfNotExist(self.dbConfig)
        .then(function () {
            ...
            return self._createMigrationsTable().then(function () {
                return self._runDatabaseUpgrades().then(function () {
                    return self._lock()
                        .then(function () {
                            if (hooks.before) {
                                debug('Before hook');
                        ...
                        .then(function () {
                            ...
                            return self._createTransaction(function (transacting) {
...
KnexMigrator.prototype.migrate = function migrate(options) {
    ...
    return database.ensureConnectionWorks(this.connection)
        .then(function () {
            return self._runDatabaseUpgrades();
        })
        .then(function () {
            return self._lock();
        })
    ...
    return Promise.each(tasks, function executeTask(task) {
        return self._beforeEach({
            ...
            if (task.config && task.config.transaction) {
                return self._createTransaction(function (txn) {
                    return task.up({
                        transacting: txn
                    });
                });
...
/**
 * @description Private helper to create the migration table.
 *
 * @TODO: https://github.com/TryGhost/knex-migrator/issues/118
 * @returns {Bluebird<R> | Bluebird<any> | * | Promise<T>}
 * @private
 */
KnexMigrator.prototype._createMigrationsTable = function _createMigrationsTable() {
    let self = this;
 ...
 * @private
 */
KnexMigrator.prototype._ensureLockTable = function _ensureLockTable() {
    let self = this;
 ...
 * @private
 */
KnexMigrator.prototype._ensureFieldLength = function _ensureFieldLength() {
 ...
 * @private
 */
KnexMigrator.prototype._ensureUniqueIndex = function _ensureUniqueIndex() {
...
/**
 * @description Private helper to run database migrations for the database tables, which knex-migrator is using.
 *
 ...
 * @private
 */
KnexMigrator.prototype._runDatabaseUpgrades = function _runDatabaseUpgrades() {
    let self = this;

    return this._ensureLockTable()
        .then(function () {
            return self._ensureFieldLength();
        })
        .then(function () {
            return self._ensureUniqueIndex();
        });
};
 ...
 * @private
 */
KnexMigrator.prototype._lock = function _lock() {
    let self = this;
    debug('Lock.');
 ...
 * @private
 */
KnexMigrator.prototype._isLocked = function _isLocked() {
 ...
 * @private
 */
KnexMigrator.prototype._unlock = function _unlock() {
    let self = this;
    debug('Unlock.');
...
/**
 * @description Private helper to create a transaction.
 * @param callback
 * @returns {*}
 * @private
 */
KnexMigrator.prototype._createTransaction = function _createTransaction(callback) {
