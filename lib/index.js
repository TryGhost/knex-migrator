...
KnexMigrator.prototype.migrate = async function migrate(options) {
    ...
    try {
        ...
        if (onlyVersion) {
            ...
            let containsVersion = Object.keys(result).find(function (key) {
        ...
        for (const [version, value] of Object.entries(result)) {
 ...
 * @private
 */
KnexMigrator.prototype._rollback = async function _rollback(options) {
    ...
    const irreversibleMigrations = tasks.filter(function (task) {
        return !!task.config?.irreversible;
    });
    if (irreversibleMigrations.length) {
        throw new errors.IrreversibleMigrationError({
    ...
    for (const task of tasks) {
        ...
        if (task.config && task.config.transaction) {
            await database.createTransaction(self.connection, function (txn) {
                return task.down({
                    transacting: txn,
                });
            });
 ...
 * @private
 */
KnexMigrator.prototype._migrateTo = async function _migrateTo(options) {
    options = options || {};
...
KnexMigrator.prototype._beforeEach = async function _beforeEach(options) {
    ...
    if (migrations.find((migration) => migration.name === task && migration.version === version)) {
