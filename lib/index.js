        ...
        .then(function (exists) {
            if (exists) {
                return self.connection.schema.alterTable('migrations', function (table) {
                    ...
                    if (err.message.match(/does not support alter column/gi)) {
                        return Promise.resolve();
                    }
        ...
        .then(function (exists) {
            if (exists) {
                return self.connection.schema.alterTable('migrations', function (table) {
                    ...
                    if (err.errno === 1061 || err.errno === 1) {
                        return Promise.resolve();
                    }

                    // if you have duplicate entries in your migration table already, we can't add the constraint
                    // ignore for now, otherwise you can't use knex-migrator anymore
                    // @TODO: this can be removed if we can clear the migrations at some point
                    if (err.errno === 1062 || err.errno === 19) {
                        return Promise.resolve();
                    }
